...
**`prefers-reduced-motion` (any width)** — a genuinely separate flow layout, not a degraded pin: no sticky, plain vertical thread, inline section headers (`.hdr-flow`), IntersectionObserver fade-ins. It keeps the width-driven sizing (desktop shell and bubble scale ≥768px, mobile sizing below) — reduced motion changes how the page moves, never how wide it is. Windows "Animation effects: off" reports this query, so a real share of desktop visitors land here; never let it collapse into a phone column. Mode is switched live on media-query change in JS.

**No scroll snap** — a mandatory section snap was tried and it traps mouse-wheel readers (nearly every Windows visitor): a wheel notch that leaves a section's hold budget lands in the seam before the next section and snaps back, so the page never advances past the hero. Trackpad flings cross the seam, which is why it looked fine on Macs. Verify wheel scrolling (100px notches) and reduced-motion at desktop widths in headless Chromium before shipping scroll-machinery changes.
