...
**Mobile (<768px) and `prefers-reduced-motion`** — a genuinely separate layout path, not a degraded pin: no sticky, plain vertical thread, 17px bubble text (real Messages size), inline section headers, IntersectionObserver fade-ins. Mode is switched live on media-query change in JS. On <768px only, the hero swaps the bubble intro for `.hero-mobile`, a port of the previous live site's mobile load: the art band blown up to 250vw and shifted so the Campanile centers, then serif headline / subhead / full-width waitlist CTA (`source: 'hero'`) / quiet FAQs link; the floating pill only surfaces once the hero is scrolled past.
