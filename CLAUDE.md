...
**Mobile (<768px) and `prefers-reduced-motion`** — a genuinely separate layout path, not a degraded pin: no sticky, plain vertical thread, 17px bubble text (real Messages size), inline section headers, IntersectionObserver fade-ins. Mode is switched live on media-query change in JS. On <768px only, the hero shows the drawing (art band blown up to 250vw, cropped to 64vw tall with a bottom mask fade, and shifted so the Campanile centers — the previous live site's mobile treatment) with the intro bubbles starting directly beneath it, then `.hero-mobile`: a full-width waitlist CTA (`source: 'hero'`) and a quiet FAQs link; the floating pill only surfaces once the hero is scrolled past.
